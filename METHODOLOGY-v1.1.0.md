...
**Valid Day.** A Collection Day that produces at least 10 Qualifying Listings after all Quality Filters are applied. Only Valid Days contribute Observations to the weekly calculation.
...
- US geography only (v1.1.0 scope)
...
### 3.4 CRI-R Reference Rate Companion Series

CCIR publishes a companion series — **CRI-R** — consisting of weekly posted rate cards collected from hyperscalers and mid-market platforms. CRI-R is published at `data/reference-rates/` in the cri-h100 repository alongside the CRI-H100 index series.

**What CRI-R is.** CRI-R collects and publishes posted on-demand and reserved-instance pricing from four sources: Azure (NC40ads H100 v5), AWS (p5.48xlarge), GCP (H100 80GB), and RunPod (spot, community, and secure tiers). Coverage spans H100, A100, H200, and V100 models — 18 rate points per weekly collection. Azure and AWS sources require no authentication and are fully independently reproducible. GCP and RunPod require a freely obtainable API key, disclosed in each collection's `.meta.json` provenance file.

**What CRI-R is not.** CRI-R rates are posted prices, not market-clearing prices. CRI-R is not an IOSCO-compliant benchmark and is not suitable for direct citation in credit agreements as a reference rate. It is published as contextual market structure data only.

**CRI-R is not an input to CRI-H100.** The CRI-H100 calculation is derived exclusively from the Vast.ai public API as specified in §4. CRI-R data has no role in the CRI-H100 calculation. Adding any CRI-R source as an input to CRI-H100 would constitute a Material Methodology Change requiring the full change control process under Governance Framework §5.2, including 60-day advance notice and 30-day stakeholder consultation.

**Analytical use.** The spread between CRI-R posted rates and CRI-H100 spot quantifies the **reliability premium** — the market price of enterprise-grade GPU access over marginal Tier 3 spot supply. This spread is useful for lenders calibrating covenant trigger levels and for analysts assessing whether Tier 1 contract renewal risk is increasing (spread compression) or stable (spread stable or widening).
...
IF len(valid_days) >= 3:
    ...
    low_confidence = (len(valid_days) < 7) OR (any_day_had_fewer_than_10_obs)
ELSE:
...
- Any single Collection Day in the window had fewer than 10 Qualifying Listings
...
CRI-H100 v1.1.0 covers **United States listings only**.
...
CRI-H100 v1.1.0 is based solely on Vast.ai marketplace data. Enterprise contracts, hyperscaler pricing, and other marketplace rates are not reflected. Vast.ai is a reproducible proxy for on-demand spot pricing but does not capture the full market.
